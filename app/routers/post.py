...
def update_post(id:int, post: schemas.PostCreate, status_code=status.HTTP_204_NO_CONTENT, db: Session = Depends(get_db), 
    ...
    if fetched_post==None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail = f"post with if: {id} does not exist")
